...
def _originals() -> dict[tuple[type, str], Any]:
    """Return the original class attributes patched by ``_instrument``."""
...
def test_uninstrument_keeps_third_party_wrappers_working(
    ...
    # Uninstrumenting must not unwrap a wrapper somebody else applied first.
    original_run = smolagents.MultiStepAgent.__dict__["run"]
    calls: list[str] = []
...
def test_completion_hook_failure_does_not_replace_agent_result(
    tracer_provider,
    logger_provider,
    meter_provider,
    span_exporter,
    ...
    with instrument(
        ...
        result = CodeAgent(tools=[], model=FakeCodeModel(), max_steps=3).run(
            "Test question"
        )
...
def test_completion_hook_failure_does_not_replace_agent_error(
    tracer_provider,
    logger_provider,
    meter_provider,
    span_exporter,
