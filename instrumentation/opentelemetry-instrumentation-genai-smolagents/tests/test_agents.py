...
def test_tool_calling_agent_with_image(
    ...
    # One operation, because the model call OpenAIModel makes is left to the
...
def test_streaming_run_close_finalizes_once(
    ...
    stream.close()
    stream.close()  # a second close must not finalize again
...
def test_run_bad_call_records_an_error_span(
    ...
    # task is required, so the call fails before the agent runs a step.
...
def test_agent_run_caches_the_signature(
    instrument_with_content, monkeypatch
) -> None:
    agent = CodeAgent(tools=[], model=FakeCodeModel(), max_steps=3)
    ...
    patch_module._cached_signature.cache_clear()
...
def test_a_failed_recording_does_not_break_a_finished_run(
    ...
    # The answer is recorded after the run is over, so a conversion that raises
    # drops the content and changes nothing else. On a streamed run it is
    # recorded while the caller iterates, where an error would surface instead
    # of the StopIteration that ends their loop.
...
def test_a_failed_recording_before_the_run_does_not_break_it(
    ...
    # The agent's tools are read before it runs, so a conversion that raises
    # there would take the run down with it.
    def raise_error(*args: Any, **kwargs: Any) -> Any:
...
def test_a_model_without_a_model_id_is_supported(
    ...
    # smolagents guards its own read of model_id (agents.py), so a model that
    # has none runs. Requiring one here would break a run that works
    # uninstrumented.
...
def test_managed_agent_run_is_recorded(
    ...
    # The manager is a CodeAgent, so it calls the managed agent from generated
    # code, which local_python_executor runs in a worker thread. That thread
    # carries none of the calling context, so the managed run starts a trace of
    # its own (README, known gaps).
...
def test_a_tool_error_the_agent_recovers_from_leaves_the_run_successful(
    ...
    # smolagents catches a tool error, feeds it back to the model as an
    # observation and keeps going, so the run succeeded and its span must not
    # report the error.
