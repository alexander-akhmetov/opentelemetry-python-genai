...
Agent tests use a fake model outside the three classes instrumented for ``chat``,
so those tests emit only ``invoke_agent`` spans.
"""
...
class ModelWithoutModelId(FakeCodeModel):
    """smolagents accepts a model without ``model_id``."""
